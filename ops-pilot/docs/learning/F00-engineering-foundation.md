...
- 回滚：F00 尚无业务数据迁移；V1 只创建 pgvector 扩展。功能已进入 `main`，如需回退应 revert 合并提交或通过后续修复提交处理，不改写共享分支历史。

本次合并后复验结束后已执行普通 `docker compose down`，所有 F00 容器与项目网络均已移除，命名卷保留。
